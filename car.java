...
public class car extends cars
{
    ...
    Subtraction value;//operatiile ce trebuie efectuate
    ...
    {   
        ...
        if(isTouching(goodresult.class) || isTouching(BadAnswer.class)){//am ales un raspuns
            ...
            if(isTouching(goodresult.class)){//am ales corect
                Actor position = (Actor)getWorld().getObjects(goodresult.class).get(0);
                //position corespunde pt raspunsul corect
                
                int x = position.getX();
                int y = position.getY();
                
                getWorld().addObject(new goodorbad("good.png"), x, y);//ai ales corect!
                ...
                Actor position = (Actor)getWorld().getObjects(BadAnswer.class).get(0);
                ...
                //ai ales gresit :(
                getWorld().addObject(new goodorbad("wrongans.png"), x, y);
            ...
            getWorld().addObject(new Text("Score : " + s, 50, 300, 1), 450, 160);
            //scorul bun
            
            if(counter == 10){//oprim jocul deoarece am avut 10 intrebari
                reset();//resetam
                Greenfoot.setWorld(new StatsWorld());//mergem sa vedem performantele
            }
            if(lane == 1){//raspunsul corect este pe prima banda
                getWorld().addObject(new goodresult(1), 1150, 410);
                getWorld().addObject(new BadAnswer(1), 1150, 525);
            }
            else{//raspunsul corect este pe a doua banda
                getWorld().addObject(new goodresult(1), 1150, 525);
                getWorld().addObject(new BadAnswer(1), 1150, 410);
            }
        }
