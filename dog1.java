...
public class dog1 extends dogs
{
    public dog1(){//adjustam imaginea
    ...
    Divide_World div_w;//lumea impartirilor
    ...
    public void touch(){//am ales raspunsul
        ...
        getWorld().removeObjects(getWorld().getObjects(goodresult.class));
        getWorld().removeObjects(getWorld().getObjects(BadAnswer.class));
        getWorld().removeObjects(getWorld().getObjects(Text.class));
    
        //am ales rezultatul bun
        if(isTouching((goodresult.class)))
            score++;
        ...
        //adjustam scorul
        getWorld().addObject(new Text("Score : " + s, 50, 300, 2), 450, 160);
                
        if(counter == 10){//jocul s-a terminat
            reset();//resetam
            div_w.start = 0;//resetam
            change = 1;//am schimbat lumea
            Greenfoot.setWorld(new StatsWorld());//intram in lumea rezultatelor
        }
        
        if(lane == 1){//rezultatul gresit se afla pe prima banda
            getWorld().addObject(new goodresult(0), 1150, 430);
            getWorld().addObject(new BadAnswer(0), 1150, 525);
        }
        else{//rezultatul gresit se afla pe a doua banda
            getWorld().addObject(new goodresult(0), 1150, 525);
            getWorld().addObject(new BadAnswer(0), 1150, 430);
        }
    ...
    Subtraction value;//clasa operatiilor ce trebuie efectuate
    ...
    public void check(){
        ...
        if(isTouching(goodresult.class) && lane == cnt){//verificam daca suntem pe banda buna
            int x = getX() + 130;
            int y = getY();
            getWorld().addObject(new goodorbad("good.png"), x, y);//ai ales bine!
            ...
            correct.setVolume(40);//scadem volumul
                ...
                int x = getX() + 130;
                int y = getY();
                getWorld().addObject(new goodorbad("wrongans.png"), x, y);//ai ales gresit:(
    ...
    public void act()
    {
        if (Greenfoot.mouseClicked(this) && dog1 == 0) {//am dat click
            ...
            Greenfoot.setWorld(new Divide_World());//lume noua
        }
        if(dog1 >= 1){//daca am ales playerul
            ...
            if(isTouching((goodresult.class)) || isTouching(BadAnswer.class)){
                //am ales un rezultat
                check();//verificam
